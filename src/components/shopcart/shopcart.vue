...
export default {
    ...
    watch: {
        totalCount () {
            this.totalCount || (this.fold = true)
        },
        fold () {
            if (!this.fold) {
                this.$nextTick(() => {
        ...
        beforeEnter (el) {
            ...
            while (count --) {
                ...
                if (ball.show) {
                    ...
                    let x = rect.left - 32; // 减去小球的偏移
                    let y = -(window.innerHeight - rect.top - 22);
                    el.style.display = ''; // 让元素显示
                    el.style.webkitTransform = `translate3d(0, ${y}px, 0)`;
                    el.style.transfrom = `translate3d(0, ${y}px, 0)`;
                    let inner = el.getElementsByClassName('inner-hook')[0]; // hook表示只用来被js选择的，没有对应样式
